#--------------------------------------------------------------------------------------
#
#    Enrichment of eQTL in ATAC-seq peaks - conditionally independent
#
#--------------------------------------------------------------------------------------
...
cat("============================")
tibble(
  variable = c("qtl_indep", "snp_file", "out_file", "peaks", "peak_dir", "cell_type", 
               "gen_pc", "exp_pc", "norm_method"),
  value    = c(qtl_indep, snp_file, out_file, peaks, peak_dir, cell_type, 
               gen_pc, exp_pc, norm_method)) |> 
...
ziffra_mapping_all <- list(
  ...
  "NPC-1"      = "RG_MACSpeaks", 
  "NPC-2"      = "RG_MACSpeaks", 
  "Glu-UL"    = "ulEN_MACSpeaks",
  "Glu-UL-0"  = "ulEN_MACSpeaks", 
  "Glu-UL-1"  = "ulEN_MACSpeaks", 
  "Glu-UL-2"  = "ulEN_MACSpeaks",
  "Glu-DL"    = "dlEN_MACSpeaks",
  "Glu-DL-0"  = "dlEN_MACSpeaks",
  "Glu-DL-1"  = "dlEN_MACSpeaks",
  "Glu-DL-2"  = "dlEN_MACSpeaks",
  "GABA"       = c("IN_CGE_MACSpeaks", "IN_MGE_MACSpeaks"),
  "GABA-0"     = c("IN_CGE_MACSpeaks", "IN_MGE_MACSpeaks"),
  "GABA-1"     = c("IN_CGE_MACSpeaks", "IN_MGE_MACSpeaks"),
  "GABA-2"     = c("IN_CGE_MACSpeaks", "IN_MGE_MACSpeaks"),
...
for (cell_type in names(cell_type_mapping)) {
  ...
  if (nrow(my_eqtl) < 5) {
    message("Skipping ", cell_type, ": only ", nrow(my_eqtl), " secondary independent eQTLs (need ≥5)")
    next
  }
...
if (nrow(ziffra_enrichment_results) == 0) {
  
  message("Cell type did not meet the >=20 secondary eQTL threshold - writing empty stub file")
  empty_results <- tibble(
    eqtl_cell_type = character(),
    peak_cell_type = character(),
    p_value = numeric(),
    observed_overlap = integer(),
    mean_permuted_overlap = numeric(),
    fold_enrichment = numeric()
    )
  
  write_rds(empty_results, out_file)
} else {
  
  message("\nSaving enrichment results to: ", out_file)
